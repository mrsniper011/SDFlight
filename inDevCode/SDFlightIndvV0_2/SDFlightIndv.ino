...
#define LOG_FIXONLY true//to be pulled from sd card/eeprom
...
int32_t latHistory[averagePoints];
int32_t lonHistory[averagePoints];
...
void loop() {
  ...
  if (GPS.newNMEAreceived()) {
    ...
    lAv /= averagePoints;
    if((dAlt*10.0) > (lAv+584) || (dAlt*10) < (lAv-584))//if current datapoint indicates an instantaneous climb/descent faster than 11,500 fpm, discard the data.
      return;
    altHistory[0] = dAlt*10;
    for(int i = 0; i < (averagePoints-1); i++){
      altHistory[i+1] = altHistory[i+2]
    }
    lAv = 0;
    for(int i = 0; i < averagePoints; i++){
      int lAv += latHistory[i];
    }
    lAv /= averagePoints;
    ...
    HaverSine(float , float lon1, float lat2, float lon2)
    //------------</indev code>
    */
